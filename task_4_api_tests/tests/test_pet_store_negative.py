...
# POST endpoint
def test_add_pet_empty_data(pet_service):
...
def test_add_pet_broken_json(pet_service):
    response = pet_service.add_pet("text")
...
def test_update_pet_broken_json(pet_service):
    response = pet_service.update_pet("text")
    assert response.status_code == 400


# GET endpoint
def test_get_non_existing_pet(pet_service):
    response = pet_service.find_pet_by_id(999999) # Assuming this ID does not exist, petStore seems to return random values
    assert response.status_code == 404
...
def test_delete_non_existing_pet(pet_service):
    response = pet_service.delete_pet(999999) # the same here, random values returned from pet store
    assert response.status_code == 404
